...
var HF_ROUTER_AUTO_ENDPOINT = `${HF_ROUTER_URL}/v1`;
...
// node_modules/@huggingface/inference/dist/esm/providers/baseten.js
...
// node_modules/@huggingface/inference/dist/esm/providers/clarifai.js
var CLARIFAI_API_BASE_URL = "https://api.clarifai.com";
var ClarifaiConversationalTask = class extends BaseConversationalTask {
  constructor() {
...
// node_modules/@huggingface/inference/dist/esm/providers/publicai.js
...
// node_modules/@huggingface/inference/dist/esm/providers/zai-org.js
...
var PROVIDERS = {
  baseten: {
    conversational: new BasetenConversationalTask()
  },
  "black-forest-labs": {
  ...
  clarifai: {
    conversational: new ClarifaiConversationalTask()
  },
  cohere: {
  ...
  publicai: {
    conversational: new PublicAIConversationalTask()
  },
  replicate: {
...
var InferenceClient = class _InferenceClient {
  ...
  constructor(accessToken = "", defaultOptions = {}) {
    ...
    for (const [name, fn] of typedEntries(tasks_exports)) {
...
// node_modules/@huggingface/tasks/dist/esm/tasks/audio-text-to-text/data.js
var taskData3 = {
  datasets: [
    {
...
function getData(type, partialTaskData = data_default19) {
  return {
    ...partialTaskData,
    id: type,
...
var TASKS_DATA = {
  "any-to-any": getData("any-to-any", data_default),
  "audio-classification": getData("audio-classification", data_default2),
  "audio-to-audio": getData("audio-to-audio", data_default4),
  "audio-text-to-text": getData("audio-text-to-text", data_default3),
  "automatic-speech-recognition": getData("automatic-speech-recognition", data_default5),
  "depth-estimation": getData("depth-estimation", data_default18),
  "document-question-answering": getData("document-question-answering", data_default6),
  "visual-document-retrieval": getData("visual-document-retrieval", data_default37),
  "feature-extraction": getData("feature-extraction", data_default7),
  "fill-mask": getData("fill-mask", data_default8),
  "graph-ml": void 0,
  "image-classification": getData("image-classification", data_default9),
  "image-feature-extraction": getData("image-feature-extraction", data_default10),
  "image-segmentation": getData("image-segmentation", data_default14),
  "image-to-image": getData("image-to-image", data_default11),
  "image-text-to-text": getData("image-text-to-text", data_default13),
  "image-to-text": getData("image-to-text", data_default12),
  "image-to-video": getData("image-to-video", data_default15),
  "keypoint-detection": getData("keypoint-detection", data_default44),
  "mask-generation": getData("mask-generation", data_default16),
  "multiple-choice": void 0,
  "object-detection": getData("object-detection", data_default17),
  "video-classification": getData("video-classification", data_default36),
  other: void 0,
  "question-answering": getData("question-answering", data_default21),
  "reinforcement-learning": getData("reinforcement-learning", data_default20),
  robotics: void 0,
  "sentence-similarity": getData("sentence-similarity", data_default22),
  summarization: getData("summarization", data_default23),
  "table-question-answering": getData("table-question-answering", data_default24),
  "table-to-text": void 0,
  "tabular-classification": getData("tabular-classification", data_default25),
  "tabular-regression": getData("tabular-regression", data_default26),
  "tabular-to-text": void 0,
  "text-classification": getData("text-classification", data_default31),
  "text-generation": getData("text-generation", data_default32),
  "text-ranking": getData("text-ranking", data_default33),
  "text-retrieval": void 0,
  "text-to-image": getData("text-to-image", data_default27),
  "text-to-speech": getData("text-to-speech", data_default28),
  "text-to-audio": void 0,
  "text-to-video": getData("text-to-video", data_default34),
  "time-series-forecasting": void 0,
  "token-classification": getData("token-classification", data_default29),
  translation: getData("translation", data_default30),
  "unconditional-image-generation": getData("unconditional-image-generation", data_default35),
  "video-text-to-text": getData("video-text-to-text", data_default45),
  "video-to-video": getData("video-to-video", data_default19),
  "visual-question-answering": getData("visual-question-answering", data_default38),
  "voice-activity-detection": void 0,
  "zero-shot-classification": getData("zero-shot-classification", data_default39),
  "zero-shot-image-classification": getData("zero-shot-image-classification", data_default40),
  "zero-shot-object-detection": getData("zero-shot-object-detection", data_default41),
  "text-to-3d": getData("text-to-3d", data_default43),
  "image-to-3d": getData("image-to-3d", data_default42)
};
...
var keras_hub = (model) => {
  ...
  const tasks2 = model.config?.keras_hub?.tasks ?? [];
  const snippets2 = [];
...
var pruna = (model) => {
  ...
  if (model.tags.includes("diffusers")) {
    snippets2 = pruna_diffusers(model);
  } else if (model.tags.includes("transformers")) {
    snippets2 = pruna_transformers(model);
  } else {
    snippets2 = pruna_default(model);
  }
...
${snippet}`;
  ...
  snippets2 = snippets2.map(ensurePrunaModelImport);
  if (model.tags.includes("pruna_pro-ai")) {
    return snippets2.map((snippet) => snippet.replace(/\bpruna\b/g, "pruna_pro").replace(/\bPrunaModel\b/g, "PrunaProModel"));
  }
  return snippets2;
};
...
var snippetGenerator = (templateName, inputPreparationFn) => {
  return (model, provider, inferenceProviderMapping, opts) => {
    ...
    const inputs = opts?.inputs ? { inputs: opts.inputs } : inputPreparationFn ? inputPreparationFn(model, opts) : { inputs: getModelInputSnippet(model) };
    const request2 = makeRequestOptionsFromResolvedModel(providerModelId, providerHelper, {
      accessToken: accessTokenOrPlaceholder,
      provider,
      endpointUrl: opts?.endpointUrl ?? (provider === "auto" ? HF_ROUTER_AUTO_ENDPOINT : void 0),
      ...inputs
    ...
    const params = {
      ...
      baseUrl: task === "conversational" && !opts?.endpointUrl && !opts?.directRequest ? HF_ROUTER_AUTO_ENDPOINT : removeSuffix(request2.url, "/chat/completions"),
      fullUrl: task === "conversational" && !opts?.endpointUrl && !opts?.directRequest ? HF_ROUTER_AUTO_ENDPOINT + "/chat/completions" : request2.url,
      inputs: {
        asObj: inputs,
...
function formatBody(obj, format2) {
  switch (format2) {
    case "curl":
